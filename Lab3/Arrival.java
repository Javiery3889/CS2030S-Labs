...
class Arrival extends Event {
  ...
  public Event[] simulate() {
    SCounter availCounter = this.shop.getAvailSCounter();
    if (availCounter != null) {
      return new Event[] {
        new ServiceBegin(this.getTime(), this.customer, availCounter, this.shop)
      };
    }
    availCounter = this.shop.getOpenCounter();
    if (availCounter != null) {
      return new Event[] {
        new JoinCounterQueue(this.getTime(), this.customer, availCounter)
      };
    }
    if (!this.shop.isQueueFull()) {
      return new Event[] {
        new JoinEntranceQueue(this.getTime(), this.customer, this.shop)
      };
    }
    return new Event[] {
