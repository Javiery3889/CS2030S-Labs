...
class MemoList<T> {
  ...
  /**
   * Lazy Implementation of Fibonacci Sequence.
   *
  ...
  public static <T> MemoList<T> generate(int n, T fst, T snd, 
    ...
    MemoList<T> memoList = new MemoList<>(new ArrayList<>());
    Memo<T> x = Memo.from(fst);
    Memo<T> y = Memo.from(snd);
    memoList.list.add(x);
    memoList.list.add(y);
  ...
  /**
   * Lazily applies Immutator in each element in list and returns MemoList.
   *
   * @param <R> The type of elements in the list.
   * @param f The immutator function on each element.
  ...
  public <R> MemoList<R> map(Immutator<? extends R, ? super T> f) {
    ...
    for (Memo<T> curr : this.list) {
      memoList.list.add(curr.transform(f));
    }
    return memoList;
  }

  /**
   * Returns an element of {@code MemoList<R>}, and flatten nested list.
   *
   * @param <R> The type of elements in the list.
   * @param f The immutator function on each element.
   * @return The resulting MemoList.
  ...
  public <R> MemoList<R> flatMap(Immutator<? extends MemoList<R>, ? super T> f) {
    ...
    for (int i = 0; i < this.list.size(); i++) {
      MemoList<R> temp = f.invoke(this.get(i));
      for (Memo<R> elem : temp.list) {
        memoList.list.add(elem);
      }
    }
    return memoList;
  }
